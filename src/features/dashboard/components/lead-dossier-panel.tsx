...
export function LeadDossierPanel({ lead, onClose, onTakeover, callEvent }: LeadDossierPanelProps) {
  ...
  return (
    <div className="flex h-full flex-col bg-white border border-border rounded-lg shadow-2xs overflow-hidden">
      ...
      <div className="flex items-center justify-between border-b border-border bg-stone-50/40 px-4 pt-2">
        <div className="border-b-2 border-stone-900 px-3 pb-2 text-xs font-semibold text-stone-900 flex items-center gap-1.5">
          <Sparkles className="h-3 w-3 text-emerald-700" />
          <span>AI Call Transcript ({callEvent?.duration || "4m 18s"})</span>
        </div>

        <Link
          href={`/leads?id=${lead.id}&tab=qualification`}
          className="pb-2 text-xs font-medium text-stone-500 hover:text-stone-900 inline-flex items-center gap-1 transition-colors group cursor-pointer"
          title="Open full Qualification & Context in Leads page"
        >
          <span>Qualification &amp; Context</span>
          <ArrowUpRight className="h-3 w-3 text-stone-400 group-hover:text-stone-700 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
        </Link>
      </div>
