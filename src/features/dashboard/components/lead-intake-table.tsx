      ...
      <Table className="min-w-[640px]">
        ...
        <TableBody>
          {leads.map((lead) => {
            ...
            const isWarm = lead.scoreCategory === "WARM";
            ...
            return (
              <TableRow key={lead.id} className="group">
                ...
                <TableCell>
                  <Badge
                    variant={isHot ? "hot" : isWarm ? "warm" : "cold"}
                    className="gap-1 text-xs"
                  >
                    <span>{lead.score}</span>
                    <span className="text-[10px] font-medium opacity-75">{lead.scoreCategory}</span>
                  </Badge>
                </TableCell>
                ...
                <TableCell className="whitespace-nowrap">
                  {lead.status === "Qualified" && (
                    <Badge variant="qualified" className="text-[11px] whitespace-nowrap">
                      {lead.status}
                    </Badge>
                  )}
                  {lead.status === "Viewing Booked" && (
                    <Badge variant="viewing" className="text-[11px] whitespace-nowrap">
                      {lead.status}
                    </Badge>
                  )}
                  {lead.status === "In Conversation" && (
                    <Badge variant="inConversation" className="text-[11px] whitespace-nowrap">
                      {lead.status}
                    </Badge>
                  )}
                  {lead.status === "Contacting" && (
                    <Badge variant="contacting" className="text-[11px] whitespace-nowrap">
                      {lead.status}
                    </Badge>
                  )}
                  {lead.status === "Follow-up" && (
                    <Badge variant="nurture" className="text-[11px] whitespace-nowrap">
                      {lead.status}
                    </Badge>
                  )}
                  {!["Qualified", "Viewing Booked", "In Conversation", "Contacting", "Follow-up"].includes(lead.status) && (
                    <span className="inline-flex items-center rounded-md border border-stone-200 bg-stone-50 px-2 py-0.5 text-[11px] font-medium text-stone-700 whitespace-nowrap">
                      {lead.status}
                    </span>
                  )}
                </TableCell>
